...
Since Jennifer did the lasso and PCR, I'm going to try ridge regression and best subset selection.
...
reg.summary$adjr2
```
For best subset selection, I am going to indirectly estimate the test error using the Cp, bic, and adjr2.
```{r}
par(mfrow=c(2,2))
plot(reg.summary$rss,xlab="Number of Variables",ylab="RSS",type="l")
plot(reg.summary$adjr2,xlab="Number of Variables",ylab="Adjusted RSq",type="l")
...
plot(regfit.full,scale="bic")
```
Two of the three measures showed an 8-variable as best. bic showed the 3-variable model as best. Therefore, I'm instead going to directly estimate the test error using cross validation (code help from: http://uc-r.github.io/model_selection).
```{r}
k <- 10
set.seed(1)
...
From our plot of cross-validation errors, it seems that a 12-variable model is ideal. Therefore, we now perform best subset selection on the full dataset.
...
Second, ridge regression with cross validation to estimate test error:
...
predict(out,type="coefficients",s=bestlam)
```
#### (b) The ridge regression is resulting in a higher test error than best subset selection. Therefore, I propose the 12-variable model from best subset selection.

#### (c) The selected model includes all but one of the features in the dataset.